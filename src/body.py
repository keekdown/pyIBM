...
class Body:
    ...
    def generate(self):
        ...
        #TODO:bug with generate points by only x axis
    ...
    def kinematics(self):
        ...
        A = 1.0
        Nf = 100
        f = 1./(Nf*Parameters.dt)
        self.x[:] = self.x0[:] + 0.0
        self.x[:] = self.y0[:] + A*sin(2*pi*f*Parameters.ite*Parameters.dt)
        self.ud[:] = 0.0
        self.vd[:] = 2*pi*f*A*cos(2*pi*f*Parameters.ite*Parameters.dt)
        self.get_neighbors()
